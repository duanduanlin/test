 ...
 * @LastEditors: dding3
 * @LastEditTime: 2025-03-15 16:12:40
 * @Description:
 * 基本的消息处理流程：
 * 1.根据消息id查表
 * 2.调用对应的处理函数
...
static int mcu_status_l3_handle(const uint8_t *data, uint8_t len, DataReader *reader)
{
    ...
    if (reader != NULL)
    {
        ...
        ChassisStatus *chassis_status = &reader->chassis_status;
        chassis_status->drv_state.mcu_state_l.fault_code = psa_can_mcu_status_l3_fault_code_decode(status->fault_code);
        ret = VCU_OK;
    }
    return ret;
}
...
static int mcu_status_r3_handle(const uint8_t *data, uint8_t len, DataReader *reader)
{
    ...
    if (reader != NULL)
    {
        ...
        ChassisStatus *chassis_status = &reader->chassis_status;
        chassis_status->drv_state.mcu_state_r.fault_code = psa_can_mcu_status_r3_fault_code_decode(status->fault_code);
        ret = VCU_OK;
    }
    return ret;
}
...
static int eps_status_handle(const uint8_t *data, uint8_t len, DataReader *reader)
{
    ...
    if (reader != NULL)
    {
        ...
        chassis_status->veh_state.eps_err_level = psa_can_eps_status_eps_angle_spd_decode(status->eps_fault_code);
        ret = VCU_OK;
    }
    return ret;
}
...
static int rcu_ctrl_cmd_handle(const uint8_t *data, uint8_t len, DataReader *reader)
{
    ...
    if (reader != NULL)
    {
        ...
        if (psa_can_rcu_ctrl_cmd_rcu_ctrl_mod_req_v_decode(cmd->rcu_ctrl_mod_req_v) == 0)
        {
            // 遥控模式
            ...
            // 转向角
            if (psa_can_rcu_ctrl_cmd_rcu_ster_per_v_decode(cmd->rcu_ster_per_v) == 0)
            {
                chassis_cmd->rcu.target_angle = psa_can_rcu_ctrl_cmd_rcu_ster_per_decode(cmd->rcu_ster_per);
            }
            // 油门大小
            if (psa_can_rcu_ctrl_cmd_rcu_acc_per_v_decode(cmd->rcu_acc_per_v) == 0)
            {
            ...
            // 驻车
            if (cmd->rcu_dbs_ctrl_v == 1) // 电子驻车拉起
            {
                chassis_cmd->rcu.park_enable = psa_can_rcu_ctrl_cmd_rcu_dbs_ctrl_decode(cmd->rcu_dbs_ctrl);
            }
            // 一键倒垃圾
            if (psa_can_rcu_ctrl_cmd_rcu_one_clk_dump_ctrl_v_decode(cmd->rcu_one_clk_dump_ctrl_v) == 0)
            {
                chassis_cmd->rcu.one_clk_dump_ctrl = psa_can_rcu_ctrl_cmd_rcu_one_clk_dump_ctrl_decode(cmd->rcu_one_clk_dump_ctrl);
            }
            //  一键清扫
            if (psa_can_rcu_ctrl_cmd_rcu_one_clk_clean_ctrl_v_decode(cmd->rcu_one_clk_clean_ctrl_v) == 0)
            {
        ...
        {
            chassis_cmd->rcu.ctrl_mod = 1; // 默认自动
        }
