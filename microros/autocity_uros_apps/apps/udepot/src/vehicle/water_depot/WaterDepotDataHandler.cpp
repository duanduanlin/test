...
namespace auto_city
{
    namespace udepot
    {
        namespace water
        {
            void WaterDepotTransportHandler::Spin()
            {
                _node->Spin();
            }

            int WaterDepotTransportHandler::Setup()
            {
                if (_is_setup)
                {
                    LOG_WARN("WaterDepotTransportHandler is already Setup\r\n");
                    return -1;
                }

                LOG_INFO("Setup WaterDepotTransportHandler\r\n");

                // auto task_request_status_bridge = std::make_shared<TaskRequestStatusBridge>();

                _node = std::make_shared<WaterDepotNode>("udepot");
                if (_node->Setup("/udepot/water_depot_cmd", "/udepot/water_depot_status") != 0)
                {
                    LOG_ERRO("depot node setup failed\r\n");
                    return -1;
                }

                _recv_msg_manager = std::make_shared<WaterDepotRecvMsgManager>();
                ...
                _send_msg_manager = std::make_shared<WaterDepotSendMsgManager>();
                ...
                _vehicle_controller = std::make_shared<WaterDepotController>();
                ;
                ...
                _cancard = std::make_shared<WaterDepotCancard>(GetParamWithDefault<std::string>("can_interfaces", "can1"));
            ...
            int WaterDepotTransportHandler::HandleRecvData()
            {
                if (!IsSetup())
                {
                    sleep(1);
            ...
            int WaterDepotTransportHandler::HandleSendData()
            {
                if (!IsSetup())
                {
                    sleep(1);
            ...
            int WaterDepotTransportHandler::HandlePubMsg()
            {
                if (!IsSetup())
                {
                    sleep(1);
