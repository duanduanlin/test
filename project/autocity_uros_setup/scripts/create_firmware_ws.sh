...
if [ $# -ge 1 ]; then
    ...
    echo "Syntax: ros2 run autocity_uros_setup create_firmware_ws.sh <package> [<platform>]"
    print_available_platforms
    exit 1
fi
...
echo "RTOS $RTOS platform $PLATFORM"
...
if [ "$ARCH" = "aarch64" ]; then
    echo "for some reason we will skip gcc-multilib g++-multilib"
    SKIP=" gcc-multilib g++-multilib"
fi
...
pushd $FW_TARGETDIR >/dev/null
    ...
    if [ $RTOS != "host" ]; then
        ros2 run autocity_uros_setup create_ws.sh $DEV_WS_DIR $PREFIX/config/$RTOS/dev_ros2_packages.txt \
        ...
        mkdir mcu_ws
        ros2 run autocity_uros_setup create_ws.sh mcu_ws $PREFIX/config/client_ros2_packages.txt $PREFIX/config/$RTOS/$TARGET_FOLDER/client_uros_packages.repos
