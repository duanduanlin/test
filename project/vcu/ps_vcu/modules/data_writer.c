 ...
 * @LastEditors: dding3
 * @LastEditTime: 2025-04-27 15:36:37
 * @Description:
...
static struct
{
    ...
    struct psa_can_vcu_odometer_t vcu_odometer;     // 里程计
    struct psa_can_vcu_nm_t vcu_nm;                 // vcu nm
    struct psa_can_vcu_fault_code_t vcu_fault_code; // vcu故障码
    struct psa_can_vcu_fault_code_t ccu_fault_code; // ccu故障码
    struct psa_can_vcu_fault_code_t mcu_fault_code; // mcu故障码
...
static int vcu_odometer_pack(CanMsg *msg);
static int vcu_nm_pack(CanMsg *msg);

static int vcu_fault_code_pack(CanMsg *msg);
static int ccu_fault_code_pack(CanMsg *msg);
static int mcu_fault_code_pack(CanMsg *msg);
...
static struct msg_send_info s_msg_send_map[] = {
    ...
    {.msg = {.id = PSA_CAN_VCU_ODOMETER_FRAME_ID,
             .len = PSA_CAN_VCU_ODOMETER_LENGTH,
             .rtr = FRAME_TYPE_DATA},
     .pack = vcu_odometer_pack,
     .net = CanNetA,
     .cycle_count = (PSA_CAN_VCU_ODOMETER_CYCLE_TIME_MS / CAN_SYNC_INTERNAL)},
    {.msg = {.id = PSA_CAN_VCU_NM_FRAME_ID,
             .len = PSA_CAN_VCU_NM_LENGTH,
             .rtr = FRAME_TYPE_DATA},
     .pack = vcu_nm_pack,
     .net = CanNetA,
     .cycle_count = (PSA_CAN_VCU_NM_CYCLE_TIME_MS / CAN_SYNC_INTERNAL)},
    {.msg = {.id = PSA_CAN_VCU_FAULT_CODE_FRAME_ID,
             .len = PSA_CAN_VCU_FAULT_CODE_LENGTH,
             .rtr = FRAME_TYPE_DATA},
     .pack = vcu_fault_code_pack,
     .net = CanNetA,
     .cycle_count = (PSA_CAN_VCU_FAULT_CODE_CYCLE_TIME_MS / CAN_SYNC_INTERNAL)},
    {.msg = {.id = PSA_CAN_VCU_FAULT_CODE_FRAME_ID,
             .len = PSA_CAN_VCU_FAULT_CODE_LENGTH,
             .rtr = FRAME_TYPE_DATA},
     .pack = ccu_fault_code_pack,
     .net = CanNetA,
     .cycle_count = (PSA_CAN_VCU_FAULT_CODE_CYCLE_TIME_MS / CAN_SYNC_INTERNAL)},
    {.msg = {.id = PSA_CAN_VCU_FAULT_CODE_FRAME_ID,
             .len = PSA_CAN_VCU_FAULT_CODE_LENGTH,
             .rtr = FRAME_TYPE_DATA},
     .pack = mcu_fault_code_pack,
     .net = CanNetA,
     .cycle_count = (PSA_CAN_VCU_FAULT_CODE_CYCLE_TIME_MS / CAN_SYNC_INTERNAL)},
...
{
    int ret = 0;
    ret = psa_can_vcu_nm_pack(msg->buffer, &s_send_data_cache.vcu_nm, msg->len);
    return ret;
}
...
{
    int ret = 0;
    s_send_data_cache.vcu_odometer.rolling_counter_0x3_f1++;
    s_send_data_cache.vcu_odometer.rolling_counter_0x3_f1 %= 0xF;
    ret = psa_can_vcu_odometer_pack(msg->buffer, &s_send_data_cache.vcu_odometer, msg->len);
    msg->buffer[msg->len - 1] = Calculate_Crc(msg->buffer, msg->len - 1);
    return ret;
}
...
{
    int ret = 0;
    ret = psa_can_vcu_fault_code_pack(msg->buffer, &s_send_data_cache.vcu_fault_code, msg->len);
    return ret;
}
...
{
    int ret = 0;
    ret = psa_can_vcu_fault_code_pack(msg->buffer, &s_send_data_cache.ccu_fault_code, msg->len);
    return ret;
}
...
{
    int ret = 0;
    ret = psa_can_vcu_fault_code_pack(msg->buffer, &s_send_data_cache.mcu_fault_code, msg->len);
    return ret;
}
...
{
    int ret = 0;
    s_send_data_cache.vcu_pwr_st.vcu_pwr_sts_roll_cnt++;
    s_send_data_cache.vcu_pwr_st.vcu_pwr_sts_roll_cnt %= 0xF;
...
static void update_fault_code(const ChassisStatus *status)
{
    if (status != NULL)
    {
        s_send_data_cache.vcu_fault_code.board_ids = status->vcu_fault_code & 0b111111;
        s_send_data_cache.vcu_fault_code.error_code = status->vcu_fault_code << 6;

        s_send_data_cache.ccu_fault_code.board_ids = status->ccu_fault_code & 0b111111;
        s_send_data_cache.ccu_fault_code.error_code = status->ccu_fault_code << 6;

        s_send_data_cache.mcu_fault_code.board_ids = status->mcu_fault_code & 0b111111;
        s_send_data_cache.mcu_fault_code.error_code = status->mcu_fault_code << 6;
    }
}

static void update_odometer(const ChassisStatus *status)
{
    s_send_data_cache.vcu_odometer.vcu_total_odometer = status->odometer.total_odometer;
    s_send_data_cache.vcu_odometer.vcu_vehicle_spd = status->drv_state.drv_veh_spd;
    s_send_data_cache.vcu_odometer.vcu_ubatt = 12;
}
...
void Sync_Chassis_Status(const ChassisStatus *status)
{
    ...
    update_fault_code(status);
    update_odometer(status);
}
...
void Sync_Drv_Cmd(const DrvCmd *cmd)
{
    ...
    {
        // left
        s_send_data_cache.mcu_cmd_l.working_mode = psa_can_mcu_cmdl_working_mode_encode(cmd->ctrl_mode);
        s_send_data_cache.mcu_cmd_l.gear_setting = psa_can_mcu_cmdl_gear_setting_encode(cmd->tgt_gear);
        s_send_data_cache.mcu_cmd_l.target_torque = psa_can_mcu_cmdl_target_torque_encode(cmd->tgt_pedpos);
        s_send_data_cache.mcu_cmd_l.target_speed = psa_can_mcu_cmdl_target_speed_encode(cmd->tgt_whl_spd_l);

        // right
        s_send_data_cache.mcu_cmd_r.working_mode = psa_can_mcu_cmdr_working_mode_encode(cmd->ctrl_mode);
        s_send_data_cache.mcu_cmd_r.gear_setting = psa_can_mcu_cmdr_gear_setting_encode(cmd->tgt_gear);
        s_send_data_cache.mcu_cmd_r.target_torque = psa_can_mcu_cmdr_target_torque_encode(cmd->tgt_pedpos);
        s_send_data_cache.mcu_cmd_r.target_speed = psa_can_mcu_cmdl_target_speed_encode(cmd->tgt_whl_spd_r);
    }
}
...
void Sync_Str_Cmd(const SteerCmd *cmd)
{
    if (cmd != NULL)
    {
        s_send_data_cache.eps_cmd.eps_angle_calibrate = psa_can_eps_cmd_eps_angle_calibrate_encode(cmd->center);
        s_send_data_cache.eps_cmd.eps_ctrl_enable = psa_can_eps_cmd_eps_ctrl_enable_encode(cmd->work_mode);
        s_send_data_cache.eps_cmd.eps_target_angle = psa_can_eps_cmd_eps_target_angle_encode(cmd->target_angle);
        s_send_data_cache.eps_cmd.eps_angle_speed = psa_can_eps_cmd_eps_angle_speed_encode(cmd->angle_speed);
    }
}
...
void Can_Msg_Sync()
{
    ...
    for (int i = 0; i < s_msg_send_map_size; i++)
    {
        if ((count % s_msg_send_map[i].cycle_count) == 0)
        {
            ...
            if (s_msg_send_map[i].pack(msg) == 0)
            {
                Data_Handle_Write(s_msg_send_map[i].net, msg);
            }
        }
    }
    count++;
