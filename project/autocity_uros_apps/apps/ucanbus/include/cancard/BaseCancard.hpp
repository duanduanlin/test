 ...
 * @LastEditors: hanson
 * @LastEditTime: 2024-09-26 16:23:53
 * @Description:
 * @FilePath: \autocity_uros_apps\apps\ucanbus\include\cancard\BaseCancard.hpp
 */
...
public:
    ...
    virtual int CanOpen()
    {
        if (_fd > 0)
        {
            LOG_WARN("can:%s is already open\r\n", _dev_name.c_str());
            return -1;
        }
        if ((_fd = socket_can_open(_dev_name.c_str())) < 0)
        {
            LOG_ERRO("CanOpen failed\r\n");
            return -1;
        }
    ...
    virtual int CanClose()
    {
        if (_fd <= 0)
        {
            LOG_WARN("can:%s is already close\r\n", _dev_name.c_str());
            return -1;
        }
