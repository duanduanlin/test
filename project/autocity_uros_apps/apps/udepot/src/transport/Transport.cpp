 ...
 * @LastEditors: hanson
 * @LastEditTime: 2024-11-13 17:34:28
 * @Description:
...
static void _pub_thread_handler()
{
    while (1)
    {
        ...
        if (node != nullptr && recv_manager != nullptr)
        {
            recv_manager->GetChassisMsg().GetMsg()->instantaneous_flow = flow_sensor_read_flow_value();
            std::this_thread::sleep_for(std::chrono::milliseconds(50));
            recv_manager->GetChassisMsg().GetMsg()->total_flow = flow_sensor_read_total_flow_value();
            // LOG_INFO("ins_clow:%f,total_flow:%f\r\n",flow_sensor_read_flow_value(),flow_sensor_read_total_flow_value());
            node->PublishWaterDepotStatus(recv_manager->GetChassisMsg());
        }
        else
        {
            LOG_WARN("node == nullptr\r\n");
        }
...
int Transport::Setup(std::shared_ptr<UdepotNode> node, std::shared_ptr<BaseVehicleFactory> vehicle_factory)
{
    if (_is_setup)
    {
    ...
    if (_vehicle_controller->Setup(_send_msg_manager->GetProtocolMap(), _node->GetWaterDepotCmdMsg(), _recv_msg_manager->GetChassisDetail()) < 0)
    {
    ...
    if(flow_sensor_init(GetParamWithDefault<std::string>("modbus_interfaces", "/dev/ttyS9").c_str(), GetParamWithDefault<int>("modbus_bitrate", 115200)) < 0)
    {
